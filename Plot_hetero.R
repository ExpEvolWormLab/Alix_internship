### Script which compute and plot heterozygotity along chromosome for each population
...
# Definine number of bin per chromosome
size_bin=500
...
compute_hist_pop <- function(df, list_bin, iter, pop, last_bin, chrom) {
  ...
  for (i in seq_len(iter - 1)) {
    ...
    df1 <- df[(list_bin[[i]]*500):(list_bin[[i + 1]]*500), ]
    # Remove POS column and stor start and end
...
for (pop in populations) {
  for (i in chromosomes) {
    ...
    bin <- size_bin
    #length_chrom <- as.numeric(df[nrow(df), "POS"])
    #list_bins <- seq(0, length_chrom, length_chrom / bin)
    nbr_snp <- nrow(df)
  ...
  g <- ggplot()+
    ...
    ggtitle(paste0("Heterozygotity along chromosome for ",pop))+
    theme_bw()+
  ...
  Results <- data.frame(Chromosome = character(0), Bin_Number = numeric(0), Bins = character(0), Number_of_heterozygotes = numeric(0))
...
for(i in length(groups)){
  pdf(file = paste0(paste0("Plots for",groups[[î]]),'.pdf'))
...
write.csv2(Results,"Heterozygotity.csv",sep = ',',quote = FALSE)
...
for (i in chrom){
  ...
  results$Chromosome <- rep(i,length(results$Start))
  HD_df <- rbind(HD_df,results)
}
