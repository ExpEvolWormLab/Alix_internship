...
while IFS= read -r i; do
    ...
    if [ -e "$i.I.g.vcf.gz" ] && [ -e "$i.II.g.vcf.gz" ] && [ -e "$i.III.g.vcf.gz" ] && [ -e "$i.IV.g.vcf.gz" ] && [ -e "$i.V.g.vcf.gz" ] && [ -e "$i.X.g.vcf.gz" ]; then
        bcftools concat "$i.I.g.vcf.gz" "$i.II.g.vcf.gz" "$i.III.g.vcf.gz" "$i.IV.g.vcf.gz" "$i.V.g.vcf.gz" "$i.X.g.vcf.gz" -O z -o $i.g.vcf.gz
	echo $i > sample.txt
